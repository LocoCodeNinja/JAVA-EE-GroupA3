...
public interface EmployeeRepository extends CrudRepository<EmployeesEntity, Integer> {
    ...
    @Query("SELECT e FROM EmployeesEntity e JOIN e.department d " +
    ...
    @Query("SELECT d.departmentId, d.departmentName, ROUND(AVG(e.salary), 2) as avgSalary FROM EmployeesEntity e " +
            "JOIN e.department d " +
